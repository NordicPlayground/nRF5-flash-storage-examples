...
static void fstorage_test(void)
{
		...
		printf("Reading from flash address 0x%X: ", (uint32_t)fs_config.p_start_addr);
		for(int i=0; i<4; i++)
		{
		...
		printf("Writing string '%s' (0x%X) to address 0x%X\r\n", char_data, data, (uint32_t)fs_config.p_start_addr + 4);
		...
		printf("Writing string '%s' (0x%X) to address 0x%X\r\n", special_char_data, data, (uint32_t)fs_config.p_start_addr + 8);
		...
		printf("Reading from flash address 0x%X (hex): ", (uint32_t)fs_config.p_start_addr);
		for(int i=0; i<4; i++)
		{
				flash_data[i] = *(fs_config.p_start_addr + i);
				printf("%X ", flash_data[i]);
		...
		printf("Reading from flash address 0x%X (ascii): ", (uint32_t)fs_config.p_start_addr);
		for(int i=0; i<4; i++)
		{
				char characters[4];
				flash_data[i] = *(fs_config.p_start_addr + i);
				...
				printf("%c%c%c%c ", characters[0], characters[1], characters[2], characters[3]);
		}
		printf("\r\n");		
}
